...
sw_inline SWint _swTypeSize(SWenum type) {
    SWint size = 0;
    switch (type) {
...
sw_inline SWuint _swGetIndex(SWenum index_type, SWuint j, const void *indices) {
    SWuint ndx = 0;
...
void swProgSetVtxAttribPointer(SWprogram *p, SWcontext *ctx, SWuint index, SWint size, SWint stride, const void *pointer) {
    ...
    SWint binded_arr_buf = ctx->binded_buffers[0];
...
void swProgDrawLinesArray(SWprogram *p, SWcontext *ctx, SWuint first, SWuint count) {
    ...
    const SWint interp_mode = (ctx->render_flags & PERSPECTIVE_CORRECTION_ENABLED) ? ((ctx->render_flags & FAST_PERSPECTIVE_CORRECTION) ? 2 : 1) : 0;
    ...
    for (j = first; j < first + count; j += 2) {
        ...
        if (!_swClipAndTransformToNDC_Line(vs_out, vs_out, p->v_out_size, num_corr_attrs)) continue;
...
void swProgDrawCurvesArray(SWprogram *p, SWcontext *ctx, SWuint first, SWuint count) {
    ...
    SWint b_depth_test = f->zbuf && (ctx->render_flags & DEPTH_TEST_ENABLED);
    SWint b_depth_write = f->zbuf && (ctx->render_flags & DEPTH_WRITE_ENABLED);
    SWfloat curve_tolerance = _swCurveTolerance(ctx, f);
    ...
    for (j = first; j < first + count; j += 4) {
        ...
        _swProcessCurve(p, f, vs_out, 0, 1, 2, 3, b_depth_test, b_depth_write, curve_tolerance);
    }
}
...
void swProgDrawTrianglesArray(SWprogram *p, SWcontext *ctx, SWuint first, SWuint count) {
    ...
    SWint b_depth_test = f->zbuf && (ctx->render_flags & DEPTH_TEST_ENABLED);
    SWint b_depth_write = f->zbuf && (ctx->render_flags & DEPTH_WRITE_ENABLED);
    SWint b_blend = (ctx->render_flags & BLEND_ENABLED);
    const SWint interp_mode = (ctx->render_flags & PERSPECTIVE_CORRECTION_ENABLED) ? ((ctx->render_flags & FAST_PERSPECTIVE_CORRECTION) ? 2 : 1) : 0;
    const SWint num_corr_attrs = (interp_mode == 0) ? 3 : p->v_out_size;
    ...
    for (j = first; j < first + count; j += 3) {
        ...
        SWint out_verts[16];
        SWint num_verts = _swClipAndTransformToNDC_Tri(vs_out, vs_out, 0, 1, 2, out_verts, p->v_out_size, num_corr_attrs);

        for (i = 1; i < num_verts - 1; i++) {
            if (interp_mode == 0) {
                _swProcessTriangle_nocorrect(p, f, vs_out,
                                             out_verts[0], out_verts[i], out_verts[i + 1],
                                             b_depth_test, b_depth_write, b_blend);
            } else if (interp_mode == 1) {
                _swProcessTriangle_correct(p, f, vs_out,
                                           out_verts[0], out_verts[i], out_verts[i + 1],
                                           b_depth_test, b_depth_write, b_blend);
            } else {
                _swProcessTriangle_fast(p, f, vs_out,
                                        out_verts[0], out_verts[i], out_verts[i + 1],
                                        b_depth_test, b_depth_write, b_blend);
            }
        }
...
void swProgDrawTriangleStripArray(SWprogram *p, SWcontext *ctx, SWuint first, SWuint count) {
    ...
    for (j = first; j < first + count - 2; j++) {
        ...
        SWint out_verts[16];
        SWint num_verts = _swClipAndTransformToNDC_Tri(vs_out, vs_out2, _0, _1, _2, out_verts, p->v_out_size, num_corr_attrs);

        for (i = 1; i < num_verts - 1; i++) {
            if (interp_mode == 0) {
                _swProcessTriangle_nocorrect(p, f, vs_out2,
                                             out_verts[0], out_verts[i], out_verts[i + 1],
                                             b_depth_test, b_depth_write, b_blend);
            } else if (interp_mode == 1) {
                _swProcessTriangle_correct(p, f, vs_out2,
                                           out_verts[0], out_verts[i], out_verts[i + 1],
                                           b_depth_test, b_depth_write, b_blend);
            } else {
                _swProcessTriangle_fast(p, f, vs_out2,
                                        out_verts[0], out_verts[i], out_verts[i + 1],
                                        b_depth_test, b_depth_write, b_blend);
            }
        }

...
LAYOUT(location = 4) in highp vec4 aVertexShUVs_0;
...
void main(void) {
    ...
    vec3 diff_color = albedo_color * (shrd_data.uSunCol.xyz * lambert * visibility + indirect_col + additional_light);

    outColor = vec4(diff_color, diff_tex_color.a);
    outNormal = vec4(0.0);

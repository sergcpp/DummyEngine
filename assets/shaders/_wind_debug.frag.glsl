...
layout(binding = REN_INST_BUF_SLOT) uniform sampler2D g_noise_texture;
...
void main(void) {
    vec3 lo_freq_bend_dir = 0.5 * texture(g_noise_texture, g_shrd_data.wind_scroll.xy +
                                          (1.0 / 256.0) * vec2(g_vtx_pos.xz)).rgb + vec3(0.5);
    vec3 hi_freq_bend_dir = 0.5 * texture(g_noise_texture, g_shrd_data.wind_scroll.zw +

...
layout(binding = LMAP_TEX_SLOTS) uniform sampler2D lm_textures[5];
...
void main() {
    ...
    while(rayQueryProceedEXT(rq)) {
        if (rayQueryGetIntersectionTypeEXT(rq, false) == gl_RayQueryCandidateIntersectionTriangleEXT) {
            ...
            RTGeoInstance geo = geometries[custom_index + geo_index];
            MaterialData mat = materials[geo.material_index];

            uint i0 = indices[geo.indices_start + 3 * prim_id + 0];
            uint i1 = indices[geo.indices_start + 3 * prim_id + 1];
            uint i2 = indices[geo.indices_start + 3 * prim_id + 2];
    ...
    if (rayQueryGetIntersectionTypeEXT(rq, true) == gl_RayQueryCommittedIntersectionNoneEXT) {
        col = clamp(RGBMDecode(textureLod(env_texture, refl_ray_ws.xyz, 0.0)), vec3(0.0), vec3(4.0)); // clamp is temporary workaround
    } else {
        ...
        col = textureLod(SAMPLER2D(mat.texture_indices[0]), uv, tex_lod).xyz;

        if ((geo.flags & RTGeoLightmappedBit) != 0u) {
            vec2 lm_uv0 = unpackHalf2x16(vtx_data1[geo.vertices_start + i0].w);
            vec2 lm_uv1 = unpackHalf2x16(vtx_data1[geo.vertices_start + i1].w);
            vec2 lm_uv2 = unpackHalf2x16(vtx_data1[geo.vertices_start + i2].w);
            ...
            vec3 normal1 = vec3(unpackSnorm2x16(vtx_data1[geo.vertices_start + i1].x), unpackSnorm2x16(vtx_data1[geo.vertices_start + i1].y).x);
            vec3 normal2 = vec3(unpackSnorm2x16(vtx_data1[geo.vertices_start + i2].x), unpackSnorm2x16(vtx_data1[geo.vertices_start + i2].y).x);
            ...
            col *= EvalSHIrradiance_NonLinear(normal,
                                              shrd_data.uProbes[geo.flags & RTGeoProbeBits].sh_coeffs[0],
                                              shrd_data.uProbes[geo.flags & RTGeoProbeBits].sh_coeffs[1],
                                              shrd_data.uProbes[geo.flags & RTGeoProbeBits].sh_coeffs[2]);
        }
    }

    imageStore(out_color_img, icoord, vec4(col, 1.0));
    imageStore(out_raylen_img, icoord, vec4(0.0));
    ...
    if (copy_horizontal) {
        ...
        imageStore(out_color_img, copy_coords, vec4(col, 0.0));
        imageStore(out_raylen_img, copy_coords, vec4(0.0));
    }
    if (copy_vertical) {

...
void main() {
    ...
    float d0 = LinearizeDepth(textureLod(depth_texture, norm_uvs, 0.0).r, g_params.clip_info);
    ...
    if (dmin < 0.05) {
        outColor = textureLod(input_texture, norm_uvs * g_params.resolution.xy / g_params.resolution.zw, 0.0);
    } else {
        if (dmin == d1) {

...
vec3 TransformVegetation(in vec3 vtx_pos_ls, vec4 vtx_color, vec4 wind_scroll, vec4 wind_params, vec4 wind_vec_ls, sampler2D _noise_tex) {
    {   // Branch/detail bending
        ...
        vec3 noise_dir_lf = wind_vec_ls.w * texture(_noise_tex, wind_scroll.xy + VEGE_NOISE_SCALE_LF * vtx_pos_ts).rgb;
        vec3 noise_dir_hf = wind_vec_ls.w * texture(_noise_tex, wind_scroll.zw + VEGE_NOISE_SCALE_HF * vtx_pos_ts).rgb;

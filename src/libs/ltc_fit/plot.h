...
class BrdfOrLTC {
public:
	BrdfOrLTC(const LTC* ltc, const Brdf* brdf, const Vec3f V = Vec3f(0,0,1), const float alpha = 1.0f) : ltc(ltc), brdf(brdf), V(V), alpha(alpha), isBrdf(brdf != NULL) {}
...
void spherical_plot(const BrdfOrLTC& brdforltc, const char* filename)
{
	...
	// camera 
	Vec3f Z = Normalize(Vec3f(-1,1,1));
	Vec3f X = Normalize(Vec3f(1,1,0));
...
void make_spherical_plots(const Brdf& brdf, const Mat3f * tab, const int N)
{
	...
	for(int t = 0 ; t<7 ; ++t)
	{
		...
		const Vec3f V(sinf(theta), 0.0f, cosf(theta));
		...
		Mat3f M = Mat3f(			
					Vec3f{LTC_matrices.linear_atXY(x, y, 0, 0), LTC_matrices.linear_atXY(x, y, 0, 1),
                                      LTC_matrices.linear_atXY(x, y, 0, 2)},
                                Vec3f{LTC_matrices.linear_atXY(x, y, 0, 3), LTC_matrices.linear_atXY(x, y, 0, 4),
                                      LTC_matrices.linear_atXY(x, y, 0, 5)},
                                Vec3f{LTC_matrices.linear_atXY(x, y, 0, 6), LTC_matrices.linear_atXY(x, y, 0, 7),

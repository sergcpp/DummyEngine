...
void SHA1::final_digest(unsigned int _digest[5])
{
    finish();

    for (int i = 0; i < 5; i++) {
        _digest[i] = (digest[i] & 0xffffffff);
    }

    const unsigned char *cc = (unsigned char *)_digest;
...
void SHA1::finish()
{
    ...
    /* Padding */
    buffer += (char)0x80;
    auto orig_size = (unsigned int)buffer.size();

    ...
    if (sequences_.back().Load(js_seq)) {
        ...
        if (js_ending.Has("choices")) {
            ...
            for (const JsElement &js_choice_el : js_choices.elements) {
                ...
                const int choice_seq_index = (int)sequences_.size();

                if (!Load(js_next_seq, read_sequence)) {
                    ctx_.log()->Error("Failed to load choise %s",
                                      js_choice_text.val.c_str());
                    return false;
                }

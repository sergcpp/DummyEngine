...
void RpDebugTextures::DrawShadowMaps(Ren::Context &ctx) {
    ...
    for (int i = 0; i < int(shadow_lists_.count); i++) {
        ...
        glBufferSubData(GL_ARRAY_BUFFER, GLintptr(prim_draw_.temp_buf1_vtx_offset()),
                        sizeof(positions), positions);
        glBufferSubData(GL_ARRAY_BUFFER,
                        GLintptr(prim_draw_.temp_buf1_vtx_offset() + sizeof(positions)),
                        sizeof(uvs), uvs);
        ...
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT,
                       (const GLvoid *)uintptr_t(prim_draw_.temp_buf_ndx_offset()));
    }
    ...
    for (int i = 0; i < int(cached_shadow_regions_.count); i++) {
        ...
        glBufferSubData(GL_ARRAY_BUFFER, GLintptr(prim_draw_.temp_buf1_vtx_offset()),
                        sizeof(positions), positions);
        glBufferSubData(GL_ARRAY_BUFFER,
                        GLintptr(prim_draw_.temp_buf1_vtx_offset() + sizeof(positions)), sizeof(uvs),
                        uvs);
    ...
    for (int i = 0; i < int(shadow_lists_.count); i++) {
        ...
        for (int j = 0; j < sh_list.view_frustum_outline_count; j += 2) {
            ...
            const float positions[] = {
                ...
                -1.0f +
                    (sh_reg.transform[1] + (p1[1] * 0.5f + 0.5f) * sh_reg.transform[3]) *
                        k,
                -1.0f + sh_reg.transform[0] + (p2[0] * 0.5f + 0.5f) * sh_reg.transform[2],
                -1.0f +
                    (sh_reg.transform[1] + (p2[1] * 0.5f + 0.5f) * sh_reg.transform[3]) *
                        k,
            };

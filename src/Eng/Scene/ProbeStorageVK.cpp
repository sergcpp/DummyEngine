...
ProbeStorage::ProbeStorage() = default;

ProbeStorage::~ProbeStorage() { Destroy(); }

bool ProbeStorage::Resize(Ren::ApiContext *api_ctx, Ren::MemoryAllocators *mem_allocs, const Ren::eTexFormat format,
                          const int res, const int capacity, Ren::ILog *log) {
    ...
    { // create new image
        ...
        img_info.format = Ren::VKFormatFromTexFormat(format);
        ...
        alloc_ = mem_allocs->Allocate(uint32_t(tex_mem_req.size), uint32_t(tex_mem_req.alignment),
                                      Ren::FindMemoryType(&api_ctx->mem_properties, tex_mem_req.memoryTypeBits,
                                                          VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT),
                                      "Probe Storage");

        const VkDeviceSize aligned_offset = Ren::AlignTo(VkDeviceSize(alloc_.alloc_off), tex_mem_req.alignment);
    ...
    this->resource_state = Ren::eResState::Undefined;

    const uint32_t BlankBlockRes = 64;

    Ren::Buffer temp_stage_buf("Temp probe stage buf", api_ctx, Ren::eBufType::Stage,
                               BlankBlockRes * BlankBlockRes * 4);
    uint8_t *blank_block = temp_stage_buf.Map(Ren::BufMapWrite);
    ...
    Ren::SmallVector<VkBufferMemoryBarrier, 1> buf_barriers;
    Ren::SmallVector<VkImageMemoryBarrier, 1> img_barriers;
    ...
    { // src buffer barrier
        ...
        new_barrier.buffer = temp_stage_buf.handle().buf;
        new_barrier.offset = VkDeviceSize(0);
        new_barrier.size = VkDeviceSize(temp_stage_buf.size());

        src_stages |= Ren::VKPipelineStagesForState(temp_stage_buf.resource_state);
        dst_stages |= Ren::VKPipelineStagesForState(Ren::eResState::CopySrc);
    }
    ...
    { // dst image barrier
        ...
        new_barrier.newLayout = Ren::VKImageLayoutForState(Ren::eResState::CopyDst);
        new_barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
        new_barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
        ...
        src_stages |= Ren::VKPipelineStagesForState(this->resource_state);
    ...
    temp_stage_buf.resource_state = Ren::eResState::CopySrc;
    this->resource_state = Ren::eResState::CopyDst;
            ...
            for (uint32_t face = 0; face < 6; face++) {
                ...
                for (uint32_t y_off = 0; y_off < _res; y_off += BlankBlockRes) {
                    ...
                    for (uint32_t x_off = 0; x_off < _res; x_off += BlankBlockRes) {
                        ...
                        vkCmdCopyBufferToImage(cmd_buf, temp_stage_buf.handle().buf, handle_.img,
...
bool ProbeStorage::SetPixelData(const int level, const int layer, const int face, const Ren::eTexFormat format,
                                const uint8_t *data, const int data_len, Ren::ILog *log) {
    if (format_ != format) {
        return false;
    }

    Ren::Buffer temp_stage_buf("Temp probe stage buf", api_ctx_, Ren::eBufType::Stage, data_len);
    {
        uint8_t *stage_data = temp_stage_buf.Map(Ren::BufMapWrite);
    ...
    vkCmdCopyBufferToImage(cmd_buf, temp_stage_buf.handle().buf, handle_.img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                           &region);

    Ren::EndSingleTimeCommands(api_ctx_->device, api_ctx_->graphics_queue, cmd_buf, api_ctx_->temp_command_pool);

    return true;
}

bool ProbeStorage::GetPixelData(const int level, const int layer, const int face, const int buf_size,
                                uint8_t *out_pixels, Ren::ILog *log) const {
...
void ProbeStorage::Destroy() {
    if (format_ != Ren::eTexFormat::Undefined) {
        ...
        handle_ = {};
        format_ = Ren::eTexFormat::Undefined;
    }
}

void ProbeStorage::Finalize() {
    VkPipelineStageFlags src_stages = 0, dst_stages = 0;
    Ren::SmallVector<VkImageMemoryBarrier, 1> img_barriers;
    ...
    if (!img_barriers.empty()) {
        ...
        this->resource_state = Ren::eResState::ShaderResource;

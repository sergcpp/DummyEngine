...
void Eng::RpOITScheduleRays::Execute(RpBuilder &builder) {
...
void Eng::RpOITScheduleRays::LazyInit(Ren::Context &ctx, Eng::ShaderLoader &sh, RpAllocBuf &vtx_buf1,
    ...
    if (!initialized) {
        ...
        if (!rp_oit_schedule_rays_.Setup(ctx.api_ctx(), {}, depth_target, ctx.log())) {
            ctx.log()->Error("[RpOITScheduleRays::LazyInit]: Failed to init render pass!");
        }
        ...
        { // VAO for simple and skinned meshes
            ...
            if (!vi_simple_.Setup(attribs, ndx_buf.ref)) {
                ctx.log()->Error("[RpOITScheduleRays::LazyInit]: vi_simple_ init failed!");
            }
        }
        ...
        { // VAO for vegetation meshes (uses additional vertex color attribute)
            ...
            if (!vi_vegetation_.Setup(attribs, ndx_buf.ref)) {
                ctx.log()->Error("[RpOITScheduleRays::LazyInit]: vi_vegetation_ init failed!");
            }
        }
        ...
        { // simple and skinned
            ...
            if (!pi_simple_[2].Init(ctx.api_ctx(), rast_state, oit_blend_simple_prog, &vi_simple_,
                ...
                ctx.log()->Error("[RpOITScheduleRays::LazyInit]: Failed to initialize pipeline!");
            }
    ...
    if (!main_draw_fb_[ctx.backend_frame()][fb_to_use_].Setup(ctx.api_ctx(), rp_oit_schedule_rays_, depth_tex.desc.w,
                                                              ...
                                                              ctx.log())) {
        ctx.log()->Error("[RpOITScheduleRays::LazyInit]: main_draw_fb_ init failed!");
    }
}
...
void Eng::RpRTReflections::Execute_HWRT_Inline(RpBuilder &builder) {
    ...
    const Ren::Pipeline &pi = pass_data_->four_bounces ? pi_rt_reflections_4bounce_inline_ : pi_rt_reflections_inline_;
    ...
    if (!initialized) {
        if (ctx.capabilities.raytracing) {
            ...
            if (ctx.capabilities.ray_query) {
                ...
                if (!pi_rt_reflections_4bounce_inline_.Init(ctx.api_ctx(), std::move(rt_reflections_inline_prog),
                                                            ctx.log())) {
                    ctx.log()->Error("RpRTReflections: Failed to initialize pipeline!");
                }
            }
        ...
        if (!pi_rt_reflections_4bounce_swrt_.Init(ctx.api_ctx(), std::move(rt_reflections_swrt_prog), ctx.log())) {
            ctx.log()->Error("RpRTReflections: Failed to initialize pipeline!");
        }

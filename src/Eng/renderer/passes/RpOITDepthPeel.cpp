...
void Eng::RpOITDepthPeel::Execute(RpBuilder &builder) {
...
void Eng::RpOITDepthPeel::LazyInit(Ren::Context &ctx, Eng::ShaderLoader &sh, RpAllocBuf &vtx_buf1, RpAllocBuf &vtx_buf2,
    ...
    if (!initialized) {
        ...
        if (!rp_depth_peel_.Setup(ctx.api_ctx(), {}, depth_target, ctx.log())) {
            ctx.log()->Error("[RpDepthPeel::LazyInit]: Failed to init render pass!");
        }
        ...
        { // VAO for simple and skinned meshes
            ...
            if (!vi_simple_.Setup(attribs, ndx_buf.ref)) {
                ctx.log()->Error("[RpDepthPeel::LazyInit]: vi_simple_ init failed!");
            }
        }
        ...
        { // VAO for vegetation meshes (uses additional vertex color attribute)
            ...
            if (!vi_vegetation_.Setup(attribs, ndx_buf.ref)) {
                ctx.log()->Error("[RpDepthPeel::LazyInit]: vi_vegetation_ init failed!");
            }
        }
        ...
        { // simple and skinned
            ...
            if (!pi_simple_[2].Init(ctx.api_ctx(), rast_state, depth_peel_simple_prog, &vi_simple_, &rp_depth_peel_, 0,
                                    ctx.log())) {
                ctx.log()->Error("[RpDepthPeel::LazyInit]: Failed to initialize pipeline!");
            }
    ...
    if (!main_draw_fb_[ctx.backend_frame()][fb_to_use_].Setup(ctx.api_ctx(), rp_depth_peel_, depth_tex.desc.w,
                                                              ...
                                                              ctx.log())) {
        ctx.log()->Error("[RpDepthPeel::LazyInit]: main_draw_fb_ init failed!");
    }
}
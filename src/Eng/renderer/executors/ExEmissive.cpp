...
void Eng::ExEmissive::Execute(FgContext &ctx) {
    ...
    FgAllocTex &color_tex = ctx.AccessRWTexture(out_color_tex_);
    FgAllocTex &depth_tex = ctx.AccessRWTexture(out_depth_tex_);

    LazyInit(ctx.ren_ctx(), ctx.sh(), vtx_buf1, vtx_buf2, ndx_buf, color_tex, depth_tex);
    DrawOpaque(ctx);
}

...
void Eng::ExGBufferFill::Execute(FgContext &fg) {
    ...
    FgAllocTex &albedo_tex = fg.AccessRWTexture(out_albedo_tex_);
    FgAllocTex &normal_tex = fg.AccessRWTexture(out_normal_tex_);
    FgAllocTex &spec_tex = fg.AccessRWTexture(out_spec_tex_);
    FgAllocTex &depth_tex = fg.AccessRWTexture(out_depth_tex_);

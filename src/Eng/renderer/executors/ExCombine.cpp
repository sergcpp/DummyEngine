...
void Eng::ExCombine::Execute(FgBuilder &builder) {
    ...
    prim_draw_.DrawPrim(PrimDraw::ePrim::Quad,
                        blit_combine_prog_[args_->compressed][args_->tonemap_mode == 2][output_tex2 != nullptr],
                        render_targets, {}, rast_state, builder.rast_state(), bindings, &uniform_params,
                        sizeof(BlitCombine::Params), 0);
}

void Eng::ExCombine::LazyInit(Ren::Context &ctx, Eng::ShaderLoader &sh) {
    if (!initialized) {

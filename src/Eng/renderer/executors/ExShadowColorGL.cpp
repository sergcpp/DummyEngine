...
void Eng::ExShadowColor::DrawShadowMaps(FgBuilder &builder) {
    ...
    rast_state.blend.src_color = uint16_t(Ren::eBlendFactor::Zero);
    rast_state.blend.dst_color = uint16_t(Ren::eBlendFactor::SrcColor);
    ...
    { // draw opaque objects
        ...
        for (int pi = 0; pi < 3; ++pi) {
            ...
            for (int i = 0; i < int((*p_list_)->shadow_lists.count); ++i) {
                ...
                j = _draw_range_ext2(builder, (*p_list_)->materials, batch_indices, (*p_list_)->shadow_batches, j,

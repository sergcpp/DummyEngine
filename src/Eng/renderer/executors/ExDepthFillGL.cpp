...
uint32_t _draw_range_ext(Eng::FgContext &ctx, const Ren::MaterialStorage *materials,
    ...
    for (; i < batch_indices.size(); i++) {
        ...
        if (!ren_ctx.capabilities.bindless_texture && batch.material_index != cur_mat_id) {
            ...
            _bind_texture4_and_sampler4(ren_ctx, mat, ctx.temp_samplers);
...
uint32_t _draw_range_ext2(Eng::FgContext &ctx, const Ren::MaterialStorage *materials,
    ...
    for (; i < batch_indices.size(); i++) {
        ...
        if (!ren_ctx.capabilities.bindless_texture && batch.material_index != cur_mat_id) {
            const Ren::Material &mat = materials->at(batch.material_index);
            _bind_texture0_and_sampler0(ren_ctx, mat, ctx.temp_samplers);
            _bind_texture4_and_sampler4(ren_ctx, mat, ctx.temp_samplers);
            cur_mat_id = batch.material_index;
        }
...
void Eng::ExDepthFill::DrawDepth(FgContext &ctx, FgAllocBuf &vtx_buf1, FgAllocBuf &vtx_buf2, FgAllocBuf &ndx_buf) {
    ...
    ctx.rast_state().scissor.enabled = false;
    ...
    { // solid meshes
        Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "STATIC-SOLID-SIMPLE");

        glBindVertexArray(pi_static_solid_[0]->vtx_input()->GetVAO());
        glUseProgram(pi_static_solid_[0]->prog()->id());

        { // one-sided
            Ren::DebugMarker _mm(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "ONE-SIDED");
        ...
        { // two-sided
            Ren::DebugMarker _mm(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "TWO-SIDED");
    ...
    { // moving solid meshes (depth and velocity)
        Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "STATIC-SOLID-MOVING");
    ...
    { // alpha-tested objects
        ...
        { // simple meshes (depth only)
            Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "STATIC-ALPHA-SIMPLE");
            ...
            { // one-sided
                ...
                i = _draw_range_ext(ctx, (*p_list_)->materials, zfill_batch_indices, zfill_batches, i,
            ...
            { // two-sided
                ...
                i = _draw_range_ext(ctx, (*p_list_)->materials, zfill_batch_indices, zfill_batches, i, DrawMask,
                                    cur_mat_id, &draws_count);
            }
        }

        { // moving meshes (depth and velocity)
            Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "STATIC-ALPHA-MOVING");
    ...
    { // static solid vegetation
        Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "VEGE-SOLID-SIMPLE");
    ...
    { // moving solid vegetation (depth and velocity)
        Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "VEGE-SOLID-MOVING");
    ...
    { // static alpha-tested vegetation
        ...
        { // static alpha-tested vegetation (depth and velocity)
            Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "VEGE-ALPHA-SIMPLE");
        ...
        { // moving alpha-tested vegetation (depth and velocity)
            Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "VEGE-ALPHA-MOVING");
    ...
    { // solid skinned meshes (depth and velocity)
        Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "SKIN-SOLID-SIMPLE");
    ...
    { // moving solid skinned (depth and velocity)
        Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "SKIN-SOLID-MOVING");
    ...
    { // static alpha-tested skinned
        ...
        { // simple alpha-tested skinned (depth and velocity)
            Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "SKIN-ALPHA-SIMPLE");
        ...
        { // moving alpha-tested skinned (depth and velocity)
            Ren::DebugMarker _m(ctx.ren_ctx().api_ctx(), ctx.cmd_buf(), "SKIN-ALPHA-MOVING");

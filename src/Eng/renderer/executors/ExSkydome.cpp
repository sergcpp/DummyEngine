...
void Eng::ExSkydomeCube::Execute(FgContext &fg) {
    ...
    FgAllocTex &transmittance_lut = fg.AccessROTexture(args_->transmittance_lut);
    FgAllocTex &multiscatter_lut = fg.AccessROTexture(args_->multiscatter_lut);
    FgAllocTex &moon_tex = fg.AccessROTexture(args_->moon_tex);
    FgAllocTex &weather_tex = fg.AccessROTexture(args_->weather_tex);
    FgAllocTex &cirrus_tex = fg.AccessROTexture(args_->cirrus_tex);
    FgAllocTex &curl_tex = fg.AccessROTexture(args_->curl_tex);
    FgAllocTex &noise3d_tex = fg.AccessROTexture(args_->noise3d_tex);
    FgAllocTex &color_tex = fg.AccessRWTexture(args_->color_tex);
...
void Eng::ExSkydomeScreen::Execute(FgContext &fg) {
    ...
    if (args_->sky_quality == eSkyQuality::Ultra) {
        FgAllocTex &transmittance_lut = fg.AccessROTexture(args_->phys.transmittance_lut);
        FgAllocTex &multiscatter_lut = fg.AccessROTexture(args_->phys.multiscatter_lut);
        FgAllocTex &moon_tex = fg.AccessROTexture(args_->phys.moon_tex);
        FgAllocTex &weather_tex = fg.AccessROTexture(args_->phys.weather_tex);
        FgAllocTex &cirrus_tex = fg.AccessROTexture(args_->phys.cirrus_tex);
        FgAllocTex &curl_tex = fg.AccessROTexture(args_->phys.curl_tex);
        FgAllocTex &noise3d_tex = fg.AccessROTexture(args_->phys.noise3d_tex);

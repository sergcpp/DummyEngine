...
vec3 get_volume_irradiance(sampler2DArray irradiance_tex, sampler2DArray distance_tex, sampler2DArray offset_tex,
                           const vec3 world_position, const vec3 surface_bias, const vec3 direction,
                           const ivec3 offset, const vec3 grid_origin, const vec3 grid_spacing) {
    ...
    const ivec3 base_probe_coords = get_base_probe_grid_coords(biased_world_position, offset, grid_origin, grid_spacing);

    // Get the world-space position of the base probe (ignore relocation)
    const vec3 base_probe_world_position = get_probe_pos_ws(base_probe_coords, offset, grid_origin, grid_spacing);
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        const int adjacent_probe_index = get_scrolling_probe_index(adjacent_probe_coords, offset);

        // Early Out: don 't allow inactive probes to contribute to irradiance
        const ivec3 adj_texel_coords = get_probe_texel_coords(adjacent_probe_index);
        if (texelFetch(offset_tex, adj_texel_coords, 0).w < 0.5) {
            continue;
        }

        // Get the adjacent probe's world position
        const vec3 adjacent_probe_world_position = get_probe_pos_ws(adjacent_probe_coords, offset, grid_origin, grid_spacing, offset_tex);
        ...
        vec3 probe_texture_uv = get_probe_uv(adjacent_probe_index, octant_coords, PROBE_DISTANCE_RES - 2);
        ...
        probe_texture_uv = get_probe_uv(adjacent_probe_index, octant_coords, PROBE_IRRADIANCE_RES - 2);

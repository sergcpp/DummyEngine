...
void main() {
    ...
    vec3 color = vec3(0.0);
    color += textureLod(g_tex, sample_positions[0], 0.0).xyz;
    color += textureLod(g_tex, sample_positions[1], 0.0).xyz;
    color += textureLod(g_tex, sample_positions[2], 0.0).xyz;
    color += textureLod(g_tex, sample_positions[3], 0.0).xyz;
    color /= 4.0;

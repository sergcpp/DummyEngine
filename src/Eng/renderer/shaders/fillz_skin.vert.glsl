...
#ifdef MOVING
    const vec3 vtx_pos_ws_prev = (model_matrix_prev * vec4(g_in_vtx_pos_prev, 1.0)).xyz;
#else
    const vec3 vtx_pos_ws_prev = (model_matrix_curr * vec4(g_in_vtx_pos_prev, 1.0)).xyz;
#endif

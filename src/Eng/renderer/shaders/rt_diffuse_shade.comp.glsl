...
void main() {
    ...
    vec3 gi_ray_ws = SampleDiffuseVector(g_tcbn_2d_tex, normal_ws, icoord, g_params.frame_index, RAND_DIM_2D_DIFFUSE_0);
    ...
    gi_ray_ws = SampleDiffuseVector(g_tcbn_2d_tex, normal_ws, icoord, g_params.frame_index, RAND_DIM_2D_DIFFUSE_1);
    ...
    if (cascade_index != -1) {
        if ((lobe_masks.bits & LOBE_SPECULAR_BIT) != 0) {
            ...
            vec3 avg_radiance = get_volume_irradiance(cascade_index, g_irradiance_tex, g_distance_tex, g_offset_tex, P, get_surface_bias(gi_ray_ws, g_shrd_data.probe_volumes[cascade_index].spacing.xyz, 0.5 * hit_t), refl_dir,
                                                      g_shrd_data.probe_volumes[cascade_index].scroll.xyz, g_shrd_data.probe_volumes[cascade_index].origin.xyz, g_shrd_data.probe_volumes[cascade_index].spacing.xyz, true);
...
#else
    const float norm_first_ray_len = NormalizeHitDist(first_ray_len, view_z, 1.0);
#endif
    imageStore(g_out_color_img, icoord, vec4(final_color, norm_first_ray_len));

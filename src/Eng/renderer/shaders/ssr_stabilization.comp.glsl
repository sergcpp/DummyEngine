...
void Stabilize(ivec2 dispatch_thread_id, ivec2 group_thread_id, uvec2 screen_size, float exposure) {
    ...
    const vec4 rad_curr = Tonemap(texelFetch(g_ssr_curr_tex, dispatch_thread_id, 0), exposure);

    vec2 hist_uvs = uvs - closest_vel;
    vec4 rad_hist = rad_curr;
    if (all(greaterThan(hist_uvs, vec2(0.0))) && all(lessThan(hist_uvs, vec2(1.0)))) {
        rad_hist = Tonemap(textureLod(g_ssr_hist_tex, hist_uvs, 0.0), exposure);
    }
    ...
    imageStore(g_out_ssr_img, dispatch_thread_id, TonemapInvert(rad, exposure));
}

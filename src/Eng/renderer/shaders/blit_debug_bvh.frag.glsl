...
void main() {
    vec2 norm_uvs = g_vtx_uvs * g_shrd_data.fren_res.zw;

    float depth = texelFetch(g_depth_tex, ivec2(g_vtx_uvs), 0).x;
    depth = 2.0 * depth - 1.0;

    vec4 ray_start_cs = vec4(g_vtx_uvs * g_shrd_data.fren_res.zw, 0.0, 1.0);
    ray_start_cs.xy = 2.0 * ray_start_cs.xy - 1.0;

    vec4 ray_end_cs = vec4(g_vtx_uvs * g_shrd_data.fren_res.zw, depth, 1.0);

...
void main() {
    ...
    if (!is_in_shadow) {
        ...
        const vec2 u = texelFetch(g_tcbn_2d_tex, (ivec3(icoord, g_params.frame_index) + 39 * RAND_DIM_2D_SHADOW) % 64, 0).xy;
        ...
        const vec3 inv_d = safe_invert(shadow_ray_ws.xyz);

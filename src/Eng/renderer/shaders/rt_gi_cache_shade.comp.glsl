...
void main() {
    ...
    if (hit_data2 == 0xffffffffu) {
        ...
        hit_t = abs(hit_t);
        vec3 throughput = UnpackRGB565(hit_data0 & 0xffffu);

        uint geo_index = (hit_data2 & 0xffu);
        if (g_params.pass_hash != 0) {
        ...
        const int tri_index = int(hit_data2 >> 8u) + int(geo.indices_start / 3);
        ...
        const float cone_width = PixelSpreadAngle * hit_t;
        ...
        if (max_component(emission_color) * g_shrd_data.cam_pos_and_exp.w > 1e-7) {
            ...
            const float mis_weight = power_heuristic(bsdf_pdf, PROBE_LIGHT_RAYS_FRACT * ls_pdf);
        ...
        for (int i = 0; i < PROBE_VOLUMES_COUNT; ++i) {
            ...
            if (weight > 0.0) {
                ...
                if ((lobe_masks.bits & LOBE_SPECULAR_BIT) != 0) {
                    ...
                    vec3 avg_radiance = get_volume_irradiance(i, g_irradiance_tex, g_distance_tex, g_offset_tex, P, get_surface_bias(N, probe_ray_dir, g_shrd_data.prev_probe_volumes[i].spacing.xyz, 0.5 * hit_t), refl_dir,
                                                              g_shrd_data.prev_probe_volumes[i].scroll.xyz, g_shrd_data.prev_probe_volumes[i].origin.xyz, g_shrd_data.prev_probe_volumes[i].spacing.xyz, true);
                    avg_radiance *= approx_spec_col * ltc.spec_t2.x + (1.0 - approx_spec_col) * ltc.spec_t2.y;
                    avg_radiance *= saturate(hit_t / (0.5 * length(g_shrd_data.prev_probe_volumes[i].spacing.xyz)));
                    light_total += GI_CACHE_MULTIBOUNCE_FACTOR * (1.0 / M_PI) * avg_radiance;
                }
                break;
            }
        }

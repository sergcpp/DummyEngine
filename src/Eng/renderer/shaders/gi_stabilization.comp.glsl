...
void Stabilize(ivec2 dispatch_thread_id, ivec2 group_thread_id, uvec2 screen_size) {
    ...
    if (all(greaterThan(hist_uvs, vec2(0.0))) && all(lessThan(hist_uvs, vec2(1.0)))) {
        rad_hist = Tonemap(textureLod(g_gi_hist_tex, hist_uvs, 0.0));
    }

    { // neighbourhood clamp
        const vec4 rad_tl = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2(-1, -1)));
        const vec4 rad_tc = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2( 0, -1)));
        const vec4 rad_tr = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2( 1, -1)));
        const vec4 rad_ml = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2(-1,  0)));
        const vec4 rad_mc = rad_curr;
        const vec4 rad_mr = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2( 1,  0)));
        const vec4 rad_bl = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2(-1,  1)));
        const vec4 rad_bc = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2( 0,  1)));
        const vec4 rad_br = Tonemap(textureLodOffset(g_gi_curr_tex, uvs, 0.0, ivec2( 1,  1)));
    ...
    imageStore(g_out_gi_img, dispatch_thread_id, TonemapInvert(rad));
}

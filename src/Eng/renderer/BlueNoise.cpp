...
void Eng::Generate1D_BlueNoiseTiles_StepFunction(const uint32_t initial_samples[]) {
    ...
    { // Scrambling optimization
        ...
        { // Sample test functions
            ...
            for (int j = 0; j < TileRes * TileRes; ++j) {
                for (float &e : errors[j / TileRes][j % TileRes]) {
        ...
        for (int y = 0; y < TileRes; ++y) {
            for (int x = 0; x < TileRes; ++x) {
                data->proximity[y][x] = calc_pixel_proximity(x, y, errors);
        ...
        for (int iter = 0; iter < MaxScramblingIterations && best_proximity < ProximityGoal; ++iter) {
            ...
            std::swap(errors[oy1][ox1], errors[oy2][ox2]);
            ...
            { // Recalc proximity around changed pixels
                for (int y = oy1 - ProximityRadius; y <= oy1 + ProximityRadius; ++y) {
                    ...
                    for (int x = ox1 - ProximityRadius; x <= ox1 + ProximityRadius; ++x) {
                        const int wrapped_x = (x + TileRes) % TileRes;
                        data->proximity[wrapped_y][wrapped_x] = calc_pixel_proximity(wrapped_x, wrapped_y, errors);
                    }
                }
            ...
            if (total_proximity > last_proximity || uniform_unorm_float(gen) < acceptance_prob) {
                ...
                if (total_proximity > best_proximity) {
                    ...
                    float min_error = FLT_MAX, max_error = 0.0f;
                    for (int j = 0; j < TileRes * TileRes; ++j) {
                        debug_errors[j / TileRes][j % TileRes] +=
                            errors[j / TileRes][j % TileRes][7 * TotalFunctionsCount / 11];
                        min_error = std::min(min_error, debug_errors[j / TileRes][j % TileRes]);
                        max_error = std::max(max_error, debug_errors[j / TileRes][j % TileRes]);
                    }
                    // normalize errors (for easier debugging)
                    for (int j = 0; j < TileRes * TileRes; ++j) {
                        float &e = debug_errors[j / TileRes][j % TileRes];
                        e = (e - min_error) / (max_error - min_error);
                    }
                    snprintf(name_buf, sizeof(name_buf), "debug_errors_%i_%i.tga", SampleCount, SampleCount);
                    WriteTGA(&debug_errors[0][0], TileRes, TileRes, TileRes, 1, 3, name_buf);
                }
            } else {
    ...
    { // Sorting optimization
        ...
        while (subset_count_pow >= 0) {
            ...
            float min_error = FLT_MAX, max_error = 0.0f;
            static std::vector<float> errors_first[TileRes][TileRes], errors_last[TileRes][TileRes];
            for (int y = 0; y < TileRes; ++y) {
                for (int x = 0; x < TileRes; ++x) {
                    errors_first[y][x].resize(TotalFunctionsCount);
                    evaluate_integrals(data->samples[y][x].data(), subset_sample_count, data->sorting_keys[y][x],
                                       errors_first[y][x].data());
                    errors_last[y][x].resize(TotalFunctionsCount);
                    evaluate_integrals(data->samples[y][x].data(), subset_sample_count,
                                       data->sorting_keys[y][x] ^ subset_sample_count, errors_last[y][x].data());
                    for (const float e : errors_first[y][x]) {
                        min_error = std::min(min_error, e);
                        max_error = std::max(max_error, e);
                    }
                    for (const float e : errors_last[y][x]) {
                        min_error = std::min(min_error, e);
                        max_error = std::max(max_error, e);
                    }
                }
            ...
            for (int j = 0; j < TileRes * TileRes; ++j) {
                for (float &e : errors_first[j / TileRes][j % TileRes]) {
                    e = (e - min_error) / (max_error - min_error);
                }
                for (float &e : errors_last[j / TileRes][j % TileRes]) {
                    e = (e - min_error) / (max_error - min_error);
                }
            }
            ...
            for (int y = 0; y < TileRes; ++y) {
                for (int x = 0; x < TileRes; ++x) {
                    proximity[y][x] = calc_pixel_proximity(x, y, errors_first, errors_last);
                    best_proximity += proximity[y][x];
                }
            }
            last_proximity = best_proximity;
            ...
            for (int iter = 0; iter < MaxSortingIterations && best_proximity < ProximityGoal; ++iter) {
                ...
                std::swap(errors_first[py][px], errors_last[py][px]);
                ...
                for (int y = py - ProximityRadius; y <= py + ProximityRadius; ++y) {
                    ...
                    for (int x = px - ProximityRadius; x <= px + ProximityRadius; ++x) {
                        const int wrapped_x = (x + TileRes) % TileRes;
                        proximity[wrapped_y][wrapped_x] =
                            calc_pixel_proximity(wrapped_x, wrapped_y, errors_first, errors_last);
                    }
                }
                ...
                if (total_proximity > last_proximity || uniform_unorm_float(gen) < acceptance_prob) {
                    ...
                    if (total_proximity > best_proximity) {
                        ...
                        for (int j = 0; j < TileRes * TileRes; ++j) {
                            debug_errors[j / TileRes][j % TileRes] +=
                                errors_first[j / TileRes][j % TileRes][7 * TotalFunctionsCount / 11];
                            min_error = std::min(min_error, debug_errors[j / TileRes][j % TileRes]);
                            max_error = std::max(max_error, debug_errors[j / TileRes][j % TileRes]);
                        }
                        // normalize errors (for easier debugging)

...
#include "brdf_ggx.h"
#include "brdf_principledClearcoat.h"
#include "brdf_principledDiffuse.h"
#include "brdf_principledSheen.h"
#include "brdf_principledSpecular.h"
...
void computeAvgTerms(const Brdf &brdf, const Vec3f &V, const float alpha, float &norm, float &fresnel,
                     Vec3f &averageDir) {
    norm = 0.0f;
    fresnel = 0.0f;
    averageDir = Vec3f(0, 0, 0);
    ...
    for (int j = 0; j < Nsample; ++j)
        for (int i = 0; i < Nsample; ++i) {
            ...
            // sample
            const Vec3f L = brdf.sample(V, alpha, U1, U2);
            ...
            if (pdf > 0) {
                ...
                // accumulate
                norm += weight;
                fresnel += weight * pow(1.0f - std::max<float>(Dot(V, H), 0.0f), 5.0f);
                averageDir += weight * L;
            }
        }
...
float computeError(const LTC &ltc, const Brdf &brdf, const Vec3f &V, const float alpha) {
    double error = 0.0;
    ...
    for (int j = 0; j < Nsample; ++j)
        for (int i = 0; i < Nsample; ++i) {
            ...
            {
                // sample
                const Vec3f L = ltc.sample(U1, U2);
...
void fit(LTC &ltc, const Brdf &brdf, const Vec3f &V, const float alpha, const float epsilon = 0.05f,
...
void fitTab(Mat3f *tab, Vec2f *tabMagFresnel, const int N, const Brdf &brdf) {
    ...
    for (int a = N - 1; a >= 0; --a)
        for (int t = 0; t <= N - 1; ++t) {
            ...
            const Vec3f V = Vec3f(sinf(theta), 0, cosf(theta));
...
float G(float w, float s, float g) { return -2.0f * sinf(w) * cosf(s) * cosf(g) + pi / 2.0f - g + sinf(g) * cosf(g); }
...
void packTab(Vec4f *tex1, Vec4f *tex2, const Mat3f *tab, const Vec2f *tabMagFresnel, const float *tabSphere, int N) {
    for (int i = 0; i < N * N; ++i) {
        const Mat3f &m = tab[i];

        Mat3f invM = Inverse(m);
        ...
        tex1[i][2] = invM[2][0];
        tex1[i][3] = invM[2][2];
        tex2[i][0] = tabMagFresnel[i][0];
        tex2[i][1] = tabMagFresnel[i][1];
        tex2[i][2] = 0.0f; // unused
        tex2[i][3] = tabSphere[i];
    }
}
...
int main(int argc, char *argv[]) {
    ...
    //BrdfPrincipledDiffuse brdf;
    //BrdfPrincipledSheen brdf;
    //BrdfPrincipledSpecular brdf;
    BrdfPrincipledClearcoat brdf;

    // allocate data
    Mat3f *tab = new Mat3f[N * N];
    Vec2f *tabMagFresnel = new Vec2f[N * N];
    ...
    Vec4f *tex1 = new Vec4f[N * N];
    Vec4f *tex2 = new Vec4f[N * N];
    ...
    writeTabC_Min(tex1, tex2, N);
    writeDDS(tab, tabMagFresnel, N);

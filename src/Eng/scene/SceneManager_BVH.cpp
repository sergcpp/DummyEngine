...
std::unique_ptr<Ren::IAccStructure> Eng::SceneManager::Build_SWRT_BLAS(const AccStructure &acc) {
    ...
    if (!scene_data_.persistent_data.swrt.rt_prim_indices_buf) {
        ...
        scene_data_.persistent_data.swrt.rt_prim_indices_buf->AddBufferView(Ren::eFormat::R32UI);
    }
    if (!scene_data_.persistent_data.swrt.rt_blas_buf) {
        ...
        scene_data_.persistent_data.swrt.rt_blas_buf->AddBufferView(Ren::eFormat::RGBA32F);
    }
...
void Eng::SceneManager::Alloc_SWRT_TLAS() {
    ...
    scene_data_.persistent_data.rt_tlas_buf[int(eTLASIndex::Main)]->AddBufferView(Ren::eFormat::RGBA32F);
    ...
    scene_data_.persistent_data.rt_tlas_buf[int(eTLASIndex::Shadow)]->AddBufferView(Ren::eFormat::RGBA32F);
    ...
    scene_data_.persistent_data.rt_tlas_buf[int(eTLASIndex::Volume)]->AddBufferView(Ren::eFormat::RGBA32F);
...
uint32_t Eng::SceneManager::ConvertToWBVH_r(Ren::Span<const gpu_light_bvh_node_t> nodes, const uint32_t node_index,
    ...
    const Ren::Vec3f scale = 2.0f / (whole_box_max + 0.001f - whole_box_min);

    uint32_t sorted_children[8] = {0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
                                   0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff};
    for (int i = 0; i < children_count; i++) {
        const Ren::Vec3f code = (children_centers[i] - whole_box_min) * scale;
...
uint32_t Eng::SceneManager::ConvertToCWBVH_r(Ren::Span<const gpu_light_bvh_node_t> nodes, const uint32_t node_index,
    ...
    // Sort children in morton order
    Ren::Vec3f children_centers[8], all_box_min = {FLT_MAX}, all_box_max = {-FLT_MAX};
    for (int i = 0; i < children_count; i++) {
        children_centers[i] = 0.5f * (nodes[children[i]].bbox_min + nodes[children[i]].bbox_max);
        if (nodes[children[i]].bbox_min[0] > -MAX_DIST) {
            all_box_min = Min(all_box_min, nodes[children[i]].bbox_min);
            all_box_max = Max(all_box_max, nodes[children[i]].bbox_max);
        }
    }

    auto quantize = [](const float v, const float min, const float max) {
        if (min == max) {
            return 0.0f;
        }
        return Ren::Clamp(255.0f * (v - min) / (max - min), 0.0f, 255.0f);
    };

    const Ren::Vec3f scale = 2.0f / (all_box_max + 0.001f - all_box_min);

    uint32_t sorted_children[8] = {0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
                                   0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff};
    for (int i = 0; i < children_count; i++) {
        const Ren::Vec3f code = (children_centers[i] - all_box_min) * scale;
    ...
    for (int i = 0; i < 8; i++) {
        if (new_children[i] != 0x7fffffff) {
            const gpu_light_bvh_node_t &child = nodes[sorted_children[i]];
...
void Eng::SceneManager::RebuildLightTree() {
    ...
    std::vector<gpu_light_cwbvh_node_t> light_wnodes;
    { // Build BVH
        std::vector<uint32_t> prim_indices;
        ...
        for (int i = 0; i < int(light_wnodes.size()); ++i) {
            if (should_remove[i]) {
                continue;
            }
            gpu_light_cwbvh_node_t &n = light_wnodes[i];
            for (int j = 0; j < 8; ++j) {
            ...
            light_wnodes[compacted_indices[i]] = n;
        }
    ...
    { // Init GPU data
        ...
        scene_data_.persistent_data.stoch_lights_buf->AddBufferView(Ren::eFormat::RGBA32UI);
        scene_data_.persistent_data.stoch_lights_nodes_buf =
        ...
        scene_data_.persistent_data.stoch_lights_nodes_buf->AddBufferView(Ren::eFormat::RGBA32F);

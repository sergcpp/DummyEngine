...
void Eng::Physics::Read(const JsObjectP &js_in, Physics &ph) {
    ...
    if (js_in.Has("pos")) {
        ...
        ph.body.pos =
            Phy::Vec3(real(js_pos[0].as_num().val), real(js_pos[1].as_num().val),
                      real(js_pos[2].as_num().val));
    } else {
        ph.body.pos = Phy::Vec3{};
    }

    if (js_in.Has("vel")) {
        const JsArrayP &js_vel = js_in.at("vel").as_arr();

        ph.body.vel_lin =
            Phy::Vec3(real(js_vel[0].as_num().val), real(js_vel[1].as_num().val),
                      real(js_vel[2].as_num().val));
    } else {
    ...
    if (js_in.Has("rot")) {
        ...
        if (js_rot.Size() == 4) { // quaternion
            ph.body.rot =
                Phy::Quat(real(js_rot[0].as_num().val), real(js_rot[1].as_num().val),
            ...
            ph.body.rot = Phy::ToQuat(real(js_rot[2].as_num().val) * ToRadians,
                                      real(js_rot[1].as_num().val) * ToRadians,
    ...
    if (js_in.Has("inv_mass")) {
        const JsNumber &js_inv_mass = js_in.at("inv_mass").as_num();
    ...
    if (js_in.Has("elasticity")) {
        const JsNumber &js_elasticity = js_in.at("elasticity").as_num();
    ...
    if (js_in.Has("friction")) {
        const JsNumber &js_friction = js_in.at("friction").as_num();
    ...
    {
        const JsObjectP &js_shape = js_in.at("shape").as_obj();
        ...
        if (js_shape_type.val == "sphere") {
            ...
            for (size_t i = 0; i < js_points.Size(); i++) {
                const JsArrayP &js_point = js_points[i].as_arr();

...
template <typename T> class MonoAlloc {
  public:
    ...
    bool operator!=(const MonoAlloc &other) const { return !(*this == other); }
    ...
    bool operator==([[maybe_unused]] const MonoAlloc &other) const { return false; }
    ...
    MonoAlloc(const MonoAlloc &other)
        : memory_(other.memory_), memory_pos_(0xffffffff), memory_size_(other.memory_size_),
          p_memory_pos_(other.p_memory_pos_) {}

    MonoAlloc &operator=(const MonoAlloc &) = delete;

    template <typename U>
    MonoAlloc(const MonoAlloc<U> &other)
        : memory_(other.memory_), memory_pos_(0xffffffff), memory_size_(other.memory_size_),
          p_memory_pos_(other.p_memory_pos_) {}
  ...
  private:
    ...
    template <class U> friend class MonoAlloc;
};
} // namespace Sys
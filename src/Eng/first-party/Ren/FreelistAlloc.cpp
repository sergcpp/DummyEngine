...
namespace Ren {
inline int fls(const uint32_t word) {
#ifdef _MSC_VER
    unsigned long index;
...
OffsetType Ren::tlsf_index_t<OffsetType, InPlace>::rounded_size(OffsetType size) {
...
template <typename OffsetType, bool InPlace>
std::pair<int, int> Ren::tlsf_index_t<OffsetType, InPlace>::mapping_insert(const OffsetType size) {
    if (size < SMALL_BLOCK_SIZE) {
...
template <typename OffsetType, bool InPlace>
std::pair<int, int> Ren::tlsf_index_t<OffsetType, InPlace>::mapping_search(const OffsetType size) {
    return mapping_insert(rounded_size(size));
}

template <typename OffsetType, bool InPlace>
OffsetType Ren::tlsf_index_t<OffsetType, InPlace>::search_suitable_block(std::pair<int, int> &inout_index) {
...
template struct Ren::tlsf_index_t<uint32_t, false>;
template struct Ren::tlsf_index_t<uint32_t, true>;
template struct Ren::tlsf_index_t<uint64_t, false>;
template struct Ren::tlsf_index_t<uint64_t, true>;
...
void Ren::FreelistAlloc::RemovePool(const uint16_t pool) {
...
void Ren::FreelistAlloc::ResizePool(uint16_t pool, uint32_t size) {
...
Ren::FreelistAlloc::Allocation Ren::FreelistAlloc::Alloc(const uint32_t size) {
...
void Ren::FreelistAlloc::Free(uint32_t block) {
...
Ren::FreelistAlloc::Range Ren::FreelistAlloc::GetFirstOccupiedBlock(const uint16_t pool) const {
...
Ren::FreelistAlloc::Range Ren::FreelistAlloc::GetNextOccupiedBlock(uint32_t block) const {
    block = all_blocks_[block].next_phys;
    if (block == 0xffffffff) {
        return {block, 0xffffffff, 0};
    }
...
bool Ren::FreelistAlloc::IntegrityCheck() const {
    int errors = 0;
...
void Ren::FreelistAlloc::insert_free_block(const uint32_t block_index, const std::pair<int, int> index) {
...
void Ren::FreelistAlloc::disconnect_free_block(const uint32_t block_index, const std::pair<int, int> index) {
...
void Ren::FreelistAlloc::block_trim_free(uint32_t block_index, uint32_t size) {
...
uint32_t Ren::FreelistAlloc::block_prepare_used(uint32_t block_index, uint32_t size) {
...
void Ren::FreelistAlloc::block_mark_as_free(const uint32_t block_index) {
    const uint32_t next = all_blocks_[block_index].next_phys;
    all_blocks_[next].is_prev_free = 1;
    all_blocks_[block_index].is_free = 1;
}

void Ren::FreelistAlloc::block_absorb(const uint32_t prev_index, const uint32_t block_index) {
...
uint32_t Ren::FreelistAlloc::block_merge_prev(uint32_t block_index) {
...
void Ren::FreelistAlloc::block_merge_next(uint32_t block_index) {
    const uint32_t next = all_blocks_[block_index].next_phys;

...
force_inline uint32x4_t _neon_unpacklo_epi8(uint32x4_t a, uint32x4_t b) {
    ...
    return vreinterpretq_u32_u8(vzip1q_u8(vreinterpretq_u8_u32(a), vreinterpretq_u8_u32(b)));
#else
...
#if defined(__aarch64__)
    return vreinterpretq_u32_u8(vzip2q_u8(vreinterpretq_u8_u32(a), vreinterpretq_u8_u32(b)));
#else
...
void Ren::CopyYChannel_16px(const uint8_t *y_src, int y_stride, int w, int h, uint8_t *y_dst) {
    ...
    if (is_input_aligned && is_output_aligned) {
        auto *py_dst = reinterpret_cast<uint32x4_t *>(__builtin_assume_aligned(y_dst, 16));
        for (int y = 0; y < h; ++y) {
            const auto *py_img = reinterpret_cast<const uint32x4_t *>(__builtin_assume_aligned(y_src, 16));
            for (int x = 0; x < w / 16; ++x) {
#if 0 //__has_builtin(__builtin_nontemporal_store)
                const uint32x4_t y_src_val = __builtin_nontemporal_load(py_img++);
                __builtin_nontemporal_store(y_src_val, py_dst++);
#else
                const uint32x4_t y_src_val = vld1q_u32(reinterpret_cast<const uint32_t *>(py_img++));
        ...
        for (int y = 0; y < h; ++y) {
            const auto *py_img = reinterpret_cast<const uint32x4_t *>(__builtin_assume_aligned(y_src, 16));
            for (int x = 0; x < w / 16; ++x) {
#if 0 //__has_builtin(__builtin_nontemporal_load)
                const uint32x4_t y_src_val = __builtin_nontemporal_load(py_img++);
#else
                const uint32x4_t y_src_val = vld1q_u32(reinterpret_cast<const uint32_t *>(py_img++));
#endif
...
void Ren::InterleaveUVChannels_16px(const uint8_t *u_src, const uint8_t *v_src, int u_stride, int v_stride, int w,
                                    int h, uint8_t *uv_dst) {
    ...
    if (is_input_aligned && is_output_aligned) {
        auto *puv_dst = reinterpret_cast<uint32x4_t *>(__builtin_assume_aligned(uv_dst, 16));
        for (int y = 0; y < h; ++y) {
            const auto *pu_img = reinterpret_cast<const uint32x4_t *>(__builtin_assume_aligned(u_src, 16));
            const auto *pv_img = reinterpret_cast<const uint32x4_t *>(__builtin_assume_aligned(v_src, 16));
            for (int x = 0; x < w / 16; ++x) {
#if 0 // __has_builtin(__builtin_nontemporal_load)
                const uint32x4_t u_src_val = __builtin_nontemporal_load(pu_img++);
                const uint32x4_t v_src_val = __builtin_nontemporal_load(pv_img++);
#else
                const uint32x4_t u_src_val = vld1q_u32(reinterpret_cast<const uint32_t *>(pu_img++));
                const uint32x4_t v_src_val = vld1q_u32(reinterpret_cast<const uint32_t *>(pv_img++));
#endif

...
namespace Gui {
template <typename TSignature> class Signal;

template <typename R, typename... Args> class Signal<R(Args... args)> {
  ...
  public:
    ...
    R FireOne(size_t i, Args... args) { return delegates_[i](args...); }
    ...
    template <R (*TFunc)(Args... args)> void Connect() {
        delegates_.emplace_back();
        delegates_.back().template Bind<TFunc>();
    }

    template <class T, R (T::*TMethod)(Args... args)> void Connect(T *p_object) {
        delegates_.emplace_back();
        delegates_.back().template Bind<T, TMethod>(p_object);
    }

    template <class T, R (T::*TMethod)(Args... args) const> void Connect(const T *p_object) {
        delegates_.emplace_back();
        delegates_.back().template Bind<T, TMethod>(p_object);
    }
};

template <typename TSignature, int N = 8> class SignalN;

template <typename R, typename... Args, int N> class SignalN<R(Args... args), N> {
  ...
  public:
    ...
    template <R (*TFunc)(Args... args)> void Connect() { delegates_[count_++].template Bind<TFunc>(); }

...
Gui::Image::Image(const Ren::ImageRegionRef &tex, const Vec2f &pos, const Vec2f &size, const BaseElement *parent)
    : BaseElement(pos, size, parent), tex_(tex) {
    const Ren::ImgParams &p = tex->params;
...
Gui::Image::Image(Ren::Context &ctx, std::string_view tex_name, const Vec2f &pos, const Vec2f &size,
    ...
    tex_ = ctx.LoadImageRegion(tex_name, {}, {}, ctx.current_cmd_buf(), &status);
    if (status == Ren::eImgLoadStatus::CreatedDefault) {
        ...
        tex_ = ctx.LoadImageRegion(tex_name, data, {}, ctx.current_cmd_buf(), &status);

...
bool RaySphere(const Vec3 &ro, const Vec3 &rd, const Vec3 &center, real radius, real &t1, real &t2);

real DistanceFromLine(const Vec3 &a, const Vec3 &b, const Vec3 &p);
real SignedDistanceFromTriangle(const Vec3 &a, const Vec3 &b, const Vec3 &c, const Vec3 &p);

int FindPointFurthestInDir(const Vec3 pts[], int count, const Vec3 &dir);
Vec3 FindPointFurthestFromLine(const Vec3 pts[], int count, const Vec3 &a, const Vec3 &b);
Vec3 FindPointFurthestFromTriangle(const Vec3 pts[], int count, const Vec3 &a, const Vec3 &b, const Vec3 &c);

struct tri_t {
    int a, b, c;
};

void BuildTetrahedron(const Vec3 verts[], int count, Vec3 tet_pts[4], tri_t tet_tris[4]);
...
bool SimplexSignedVolumes(const point_t pts[], int pts_count, Vec3 &new_dir, Vec4 &out_lambdas);
...
int FindTriangleClosestToOrigin(const tri_t tris[], int tris_count, const point_t points[]);

int RemoveTrianglesFacingPoint(const Vec3 &pt, std::vector<tri_t> &tris, const point_t points[]);

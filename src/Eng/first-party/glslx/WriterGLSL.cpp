...
void glslx::WriterGLSL::Write_Assignment(const ast_assignment_expression *expression, const assoc_t parent,
    ...
    Write_Expression(expression->operand1, {prec, eAssoc::Right, eOperator::none, false}, out_stream);
    out_stream << " " << g_operators[int(expression->oper)].string << " ";
    Write_Expression(expression->operand2, {prec, eAssoc::Right, eOperator::none, true}, out_stream);
    if (need_parens) {
        out_stream << ")";
    }
}
...
void glslx::WriterGLSL::Write_PragmaDirective(const ast_pragma_directive *pragma, std::ostream &out_stream) {
    out_stream << "#pragma " << pragma->name;
    for (const char *arg : pragma->arguments) {
        out_stream << " " << arg;
    }
    out_stream << "\n";
}
...
void glslx::WriterGLSL::Write(const TrUnit *tu, std::ostream &out_stream) {
    ...
    for (int i = 0; i < int(tu->pragmas.size()); ++i) {
        Write_PragmaDirective(tu->pragmas[i], out_stream);
    }

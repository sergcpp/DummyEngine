...
void _swProcessScanline_Ref(SWuint *coverage, SWint left_offset, SWint right_offset,
                            SWint left_event, SWint left_count, SWint right_event,
                            SWint right_count, const SWint events[3], SWint tile_ndx) {
    ...
    while (1) {
        ...
        coverage[tile_ndx] |= coverage_mask;
...
void _swRasterizeTriangle_Ref(SWcull_ctx *ctx, SWint tile_row_ndx, SWint tile_mid_row_ndx,
                              SWint tile_end_row_ndx, SWint bb_width,
                              SWint slope_tile_delta[3], SWint event_start[3],
                              SWint mid_vtx_right, SWint use_tight_traversal) {
...
#define UPDATE_TILE_EVENTS_Y(i) tri_event[i] += tri_slope_tile_delta[i];
    ...
    if (tile_row_ndx < tile_mid_row_ndx) {
        ...
        while (tile_row_ndx < tile_stop_ndx) {
            ...
            _swProcessScanline_Ref(ctx->coverage, start, end, 2 /* left_event */, 1,
                                   0 /* right_event*/, 1, tri_event, tile_row_ndx);
        ...
        if (tile_row_ndx < tile_end_row_ndx) {
            ...
            if (mid_vtx_right) {
                _swProcessScanline_Ref(ctx->coverage, start, end, 2 /* left_event */, 1,
                                       0 /* right_event*/, 2, tri_event, tile_row_ndx);
            } else {
                _swProcessScanline_Ref(ctx->coverage, start, end, 2 /* left_event */, 2,
                                       0 /* right_event*/, 1, tri_event, tile_row_ndx);
            }
        ...
        if (tile_row_ndx < tile_end_row_ndx) {
            ...
            while (1) {
                ...
                _swProcessScanline_Ref(
                    ctx->coverage, start, end, mid_vtx_right + 1 /* left_event */, 1,
                    mid_vtx_right + 0 /* right_event*/, 1, tri_event, tile_row_ndx);
...
void _swProcessTriangle_Ref(SWcull_ctx *ctx, SWfloat v0[3], SWfloat v1[3],
                            SWfloat v2[3]) {
    SWzbuffer *zbuf = &ctx->zbuf;

    SWint ip0[2], ip1[2], ip2[2];
    ...
    bb_min[0] = sw_max(sw_min(ip0[0], sw_min(ip1[0], ip2[0])), 0);
    bb_min[1] = sw_max(sw_min(ip0[1], sw_min(ip1[1], ip2[1])), 0);
    bb_max[0] = sw_min(sw_max(ip0[0], sw_max(ip1[0], ip2[0])), zbuf->w - 1);
    bb_max[1] = sw_min(sw_max(ip0[1], sw_max(ip1[1], ip2[1])), zbuf->h - 1);
    ...
    SWfloat horizontal_slope_delta = ((SWfloat)zbuf->w) + 2.0f * (1.0f + 1.0f);
    if (edges[0][1] == 0.0f) {
    ...
    { // Rasterize triangle
        ...
        _swRasterizeTriangle_Ref(ctx, tile_row_ndx, tile_mid_row_ndx, tile_end_row_ndx,
                                 bb_width, slope_tile_delta, event_start, mid_vtx_right,
                                 tight_traversal);
    }
}
...
void _swProcessTrianglesIndexed_Ref(SWcull_ctx *ctx, const void *attribs,
    ...
    while (tris_count || clip_head != clip_tail) {
            ...
            for (SWint plane = 0; plane < _PlanesCount; plane++) {
                ...
                for (SWint i = 0; i < 3; i++) {
                    ...
                    case Bottom:
                        ...
                        break;
                    case Near:
                        plane_dp[i] = vW[i] - 0.0f /* near_dist */;
                        break;
                    }
                }
        ...
        if (tri_mask) {
            ...
            _swProcessTriangle_Ref(ctx, v0, v1, v2);
        }
    }
}
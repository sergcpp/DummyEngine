...
void _swProcessCurveRecursive(SWprogram *p, SWframebuffer *f,
    ...
    if ((d2 + d3) * (d2 + d3) < tolerance * (dx * dx + dy * dy)) {
        ...
        _swProcessCurveRecursive(p, f, p1, p12, p123, p1234, b_depth_test, b_depth_write, tolerance);
        _swProcessCurveRecursive(p, f, p1234, p234, p34, p4, b_depth_test, b_depth_write, tolerance);
    }
}
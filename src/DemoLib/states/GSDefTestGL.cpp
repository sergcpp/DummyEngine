...
inline float radians(float angle_grad) {
    return angle_grad * Pi / 180.0f;
}
}
...
void GSDefTest::DrawInternal() {
    ...
    for (const auto &l : lights_) {
        ...
        world_from_object = Ren::Translate(world_from_object, l.pos);
        //world_from_object = rotate(world_from_object, radians(90.0f), { 0.0f, 0.0f, 1.0f });

        Ren::Mat4f view_from_object = view_from_world * world_from_object,
                   proj_from_object = proj_from_view * view_from_object;

        glUniform3f(unif(p, U_LIGHT_POS), l.pos[0], l.pos[1], l.pos[2]);
        glUniform3f(unif(p, U_LIGHT_COL), l.col[0], l.col[1], l.col[2]);
...
void GSDefTest::InitInternal(int w, int h) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RG16F, w, h, 0, GL_RGB, GL_FLOAT, NULL);

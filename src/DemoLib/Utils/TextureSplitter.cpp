...
int TextureSplitter::Allocate(const int res[2], int pos[2]) {
...
bool TextureSplitter::Free(const int pos[2]) {
    int i = Find_Recursive(0, pos);
    return Free(i);
}

bool TextureSplitter::Free(int i) {
...
int TextureSplitter::FindNode(const int pos[2], int size[2]) const {
    int i = Find_Recursive(0, pos);
    if (i != -1) {
...
int TextureSplitter::Insert_Recursive(int i, const int res[2]) {
...
int TextureSplitter::Find_Recursive(int i, const int pos[2]) const {
    if (nodes_[i].is_free ||
...
void TextureSplitter::SafeErase(int i, int *indices, int num) {

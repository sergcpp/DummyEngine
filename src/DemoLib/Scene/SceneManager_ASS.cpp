...
bool SceneManager::PrepareAssets(const char *in_folder, const char *out_folder, const char *platform, Sys::ThreadPool *p_threads) {
    ...
    std::vector<std::pair<std::string, std::string>> shader_constants;

    shader_constants.emplace_back("$ModifyWarning", "/**********************************************/\n"
                                                    "/* This file is autogenerated, do not modify! */\n"
                                                    "/**********************************************/");

    shader_constants.emplace_back("$FltEps",        "0.0000001");

    shader_constants.emplace_back("$ItemGridResX",  AS_STR(REN_GRID_RES_X));
    ...
    shader_constants.emplace_back("$SpecTexSlot",   AS_STR(REN_SPEC_TEX_SLOT));
    shader_constants.emplace_back("$ShadTexSlot",   AS_STR(REN_SHAD_TEX_SLOT));
    shader_constants.emplace_back("$LmapDirSlot",   AS_STR(REN_LMAP_DIR_SLOT));
    shader_constants.emplace_back("$LmapIndirSlot", AS_STR(REN_LMAP_INDIR_SLOT));
    shader_constants.emplace_back("$LmapSHSlot",    AS_STR(REN_LMAP_SH_SLOT));
    shader_constants.emplace_back("$DecalTexSlot",  AS_STR(REN_DECAL_TEX_SLOT));
    shader_constants.emplace_back("$SSAOTexSlot",   AS_STR(REN_SSAO_TEX_SLOT));
    shader_constants.emplace_back("$LightBufSlot",  AS_STR(REN_LIGHT_BUF_SLOT));
    shader_constants.emplace_back("$DecalBufSlot",  AS_STR(REN_DECAL_BUF_SLOT));
    shader_constants.emplace_back("$CellsBufSlot",  AS_STR(REN_CELLS_BUF_SLOT));
    shader_constants.emplace_back("$ItemsBufSlot",  AS_STR(REN_ITEMS_BUF_SLOT));

    // Shader output channels
    shader_constants.emplace_back("$OutColorIndex", AS_STR(REN_OUT_COLOR_INDEX));
    ...
    auto inline_constants = [&shader_constants](std::string &line) {
        ...
        while ((n = line.find('$', n)) != std::string::npos) {
            for (const auto &val : shader_constants) {
                size_t p;
                if ((p = line.find(val.first, n)) != std::string::npos) {
                    line.replace(p, val.first.length(), val.second);
                    break;
                }
            }
        }

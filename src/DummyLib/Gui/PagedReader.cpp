...
const float PageMarginLeft = 0.05f, PageMarginRight = 0.0f, PageMarginTop = 0.1f,
            PageMarginBottom = 0.05f;
...
PagedReader::PagedReader(Ren::Context &ctx, const Gui::Vec2f &pos, const Gui::Vec2f &size,
                         const BaseElement *parent,
                         std::shared_ptr<Gui::BitmapFont> main_font,
    ...
    : BaseElement(pos, size, parent), parent_(parent), log_(ctx.log()),
      main_font_(std::move(main_font)), emph_font_(std::move(emph_font)),
      caption_font_(std::move(caption_font)), cur_chapter_(0), cur_page_(0) {
    using namespace PagedReaderInternal;
    background_small_.reset(new Gui::Image9Patch{ctx, Frame01, Ren::Vec2f{3.0f, 3.0f},
                                                 1.0f, Ren::Vec2f{0.0f, 0.0f},
...
bool PagedReader::LoadBook(const JsObject &js_book, const char *src_lang,
                           const char *trg_lang) {
    Clear();
    ...
    if (js_book.Has("title")) {
        ...
        const JsString &js_src_title = js_title.at(src_lang).as_str(),
    ...
    if (js_book.Has("chapters")) {
        ...
        for (const JsElement &js_chapter_el : js_chapters.elements) {
            ...
            ChapterData &chapter_src = chapters_[0].back(),
...
void PagedReader::DrawHint(Gui::Renderer *r, const Ren::Vec2f &pos,
    ...
    if (sentence_to_translate_ != -1) {
        ...
        while (char_pos < sent.end_pos) {
            ...
            if (unicode == Gui::g_unicode_less_than) {
                ...
                while (unicode != Gui::g_unicode_greater_than) {
                    char_pos +=
                        Gui::ConvChar_UTF8_to_Unicode(&input_src_data[char_pos], unicode);
        ...
        const float width = main_font_->GetWidth(portion_buf, -1, parent),
                    height = main_font_->height(parent);

        background_small_->Resize(pos - Ren::Vec2f{0.025f, 0.025f},
                                  Ren::Vec2f{width + 0.05f, height + 0.05f}, parent);
...
void PagedReader::DrawCurrentPage(Gui::Renderer *r) const {
    ...
    const float main_font_height = main_font_->height(parent_),
                emph_font_height = emph_font_->height(parent_);
    const float x_start =
                    dims_[0][0] + ((cur_page_ % 2) ? PageMarginRight : PageMarginLeft),
                x_limit = dims_[0][0] + dims_[1][0] -
                          ((cur_page_ % 2) ? PageMarginLeft : PageMarginRight),
    ...
    if (cur_page_ == 0) {
        caption_font_->DrawText(r, chapter.caption.c_str(),
                                Ren::Vec2f{x_offset, y_offset}, color_white, parent_);
    ...
    for (int sentence = page.sentence_beg; sentence < page.sentence_end; sentence++) {
        ...
        for (int char_pos = std::max(sent.start_pos, page.start_pos);
            ...
            bool draw = false, new_line = false, paragraph = false,
                 set_emphasize = false /*, pop_color = false*/;
            ...
            if (unicode == Gui::g_unicode_spacebar) {
                ...
                while (input_src_data[next_pos]) {
                    ...
                    uint32_t _unicode;
                    next_pos += Gui::ConvChar_UTF8_to_Unicode(&input_src_data[next_pos],
                                                              _unicode);

                    // skip tag
                    if (_unicode == Gui::g_unicode_less_than) {
                        while (_unicode != Gui::g_unicode_greater_than) {
                            next_pos += Gui::ConvChar_UTF8_to_Unicode(
                                &input_src_data[next_pos], _unicode);
                        }
                        continue;
                    }
                ...
                const float x_end =
                    x_offset + main_font_->GetWidth(portion_buf, -1, parent_);
                if (x_end > x_limit) {
                    new_line = true;
                }
            ...
            if (draw) {
                ...
                const Gui::BitmapFont *font =
                    emphasize ? emph_font_.get() : main_font_.get();
                // const Gui::BitmapFont *font = (sentence % 2) ? emph_font_.get() :
                // main_font_.get();
                x_offset += font->DrawText(r, portion_buf, Gui::Vec2f{x_offset, y_offset},
                                           cur_color, parent_);
    ...
    if (portion_buf_size) {
        ...
        x_offset += main_font_->DrawText(
            r, portion_buf, Gui::Vec2f{x_offset, y_offset},
            ...
            while (input_src_data[char_pos]) {
                ...
                while (input_src_data[char_pos]) {
                    ...
                    if (unicode == Gui::g_unicode_less_than) {
                        ...
                        while (unicode != Gui::g_unicode_greater_than) {
                            char_pos += Gui::ConvChar_UTF8_to_Unicode(
                                &input_src_data[char_pos], unicode);
                            tag_str[tag_str_len++] = (char)unicode;
                        }
                        tag_str[tag_str_len - 1] = '\0';
                    ...
                    if (draw) {
                        ...
                        const float text_width =
                            main_font_->GetWidth(portion_buf, -1, parent_);
...
void PagedReader::Press(const Gui::Vec2f &p, bool push) {
    ...
    for (int sentence = page.sentence_beg; sentence < page.sentence_end; sentence++) {
        ...
        for (int rect_index = sent.rect_beg; rect_index < sent.rect_end; rect_index++) {
            ...
            if (p[0] > rect.dims[0][0] && p[1] > rect.dims[0][1] &&

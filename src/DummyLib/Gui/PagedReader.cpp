...
namespace PagedReaderInternal {
    const float
        PageMarginLeft      = 0.05f,
        PageMarginRight     = 0.0f,
        PageMarginTop       = 0.1f,
...
void PagedReader::DrawCurrentPage(Gui::Renderer *r) const {
    ...
    const float
        main_font_height = main_font_->height(parent_),
        emph_font_height = emph_font_->height(parent_);
    const float
        x_start = dims_[0][0] + ((cur_page_ % 2) ? PageMarginRight : PageMarginLeft),
    ...
    if (cur_page_ < 0 || cur_page_  >= (int)chapter.pages.size()) return;
    ...
    for (int sentence = page.sentence_beg; sentence < page.sentence_end; sentence++) {
        ...
        for (int char_pos = std::max(sent.start_pos, page.start_pos); char_pos < std::min(sent.end_pos, page.end_pos); ) {
            int char_start = char_pos;
            ...
            if (unicode == Gui::g_unicode_spacebar) {
                ...
                while (input_src_data[next_pos]) {
                    ...
                    if (_unicode == Gui::g_unicode_spacebar) break;
                }
            ...
            if (draw) {
                ...
                //const Gui::BitmapFont *font = (sentence % 2) ? emph_font_.get() : main_font_.get();
    ...
    if (portion_buf_size) {
        ...
        x_offset += main_font_->DrawText(r, portion_buf, Gui::Vec2f{ x_offset, y_offset }, /*text_color_stack[text_color_stack_size - 1]*/ color_white, parent_);
    }

...
void DebugInfoUI::UpdateInfo(const FrontendInfo &frontend_info,
    ...
    back_info_smooth_.depth_opaque_pass_time_ms +=
        k * us_to_ms(backend_info.depth_opaque_pass_time_us);
    ...
    back_info_smooth_.opaque_pass_time_ms +=
        k * us_to_ms(backend_info.opaque_pass_time_us);
    back_info_smooth_.transp_pass_time_ms *= alpha;
    back_info_smooth_.transp_pass_time_ms +=
        k * us_to_ms(backend_info.transp_pass_time_us);
    ...
    back_info_smooth_.cpu_total_ms += k * us_to_ms(backend_info.cpu_end_timepoint_us -
                                                   backend_info.cpu_start_timepoint_us);
    back_info_smooth_.gpu_total_ms *= alpha;
    back_info_smooth_.gpu_total_ms += k * us_to_ms(backend_info.gpu_end_timepoint_us -
                                                   backend_info.gpu_start_timepoint_us);
    ...
    back_info_smooth_.depth_fill_draw_calls_count +=
...
void DebugInfoUI::Draw(Gui::Renderer *r) {
    ...
    { // renderer frontend performance
        ...
        font_->DrawText(r, delimiter, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, "   occ_rast: %.3f ms",
                front_info_smooth_.occluders_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, "main_gather: %.3f ms",
                front_info_smooth_.main_gather_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, "shad_gather: %.3f ms",
                front_info_smooth_.shadow_gather_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, "    sorting: %.3f ms",
                front_info_smooth_.drawables_sort_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, "item_assign: %.3f ms",
                front_info_smooth_.items_assignment_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);
    ...
    { // renderer backend performance
        ...
        vertical_offset -= font_height;
        sprintf(text_buffer, " draw_calls: [%.1f, %.1f, %.1f]",
                back_info_smooth_.shadow_draw_calls_count,
                back_info_smooth_.depth_fill_draw_calls_count,
                back_info_smooth_.opaque_draw_calls_count);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);
        ...
        vertical_offset -= font_height;
        sprintf(text_buffer, " depth_fill: %.3f ms",
                back_info_smooth_.depth_opaque_pass_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);
        ...
        vertical_offset -= font_height;
        sprintf(text_buffer, "opaque_pass: %.3f ms",
                back_info_smooth_.opaque_pass_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, "transp_pass: %.3f ms",
                back_info_smooth_.transp_pass_time_ms);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);
    ...
    if (render_flags_ & (DebugLights | DebugDecals)) {
        ...
        sprintf(text_buffer, "lights_data: %.3f kb",
                items_info_smooth_.light_sources_count * sizeof(LightSourceItem) /
                    1024.0f);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);
        ...
        vertical_offset -= font_height;
        sprintf(text_buffer, "decals_data: %.3f kb",
                items_info_smooth_.decals_count * sizeof(DecalItem) / 1024.0f);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, " cells_data: %.3f kb",
                REN_CELLS_COUNT * sizeof(CellData) / 1024.0f);
        font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset}, text_color,
                        parent_);

        vertical_offset -= font_height;
        sprintf(text_buffer, " items_data: %.3f kb",
    ...
    if (render_flags_ & DebugTimings) {
        if (prev_timing_info_.front_end_timepoint_us) {
            auto prev_front_start = double(prev_timing_info_.front_start_timepoint_us),
                 ...
                 prev_back_cpu_start =
                     double(prev_timing_info_.back_cpu_start_timepoint_us),
                 prev_back_cpu_end = double(prev_timing_info_.back_cpu_end_timepoint_us),
                 prev_back_gpu_start =
                     double(prev_timing_info_.back_gpu_start_timepoint_us),
                 prev_back_gpu_end = double(prev_timing_info_.back_gpu_end_timepoint_us),
                 prev_swap_start =
                     double(prev_timing_info_.swap_interval.start_timepoint_us),
                 ...
                 next_back_cpu_start =
                     double(cur_timing_info_.back_cpu_start_timepoint_us),
                 next_back_cpu_end = double(cur_timing_info_.back_cpu_end_timepoint_us),
                 next_back_gpu_start =
                     double(cur_timing_info_.back_gpu_start_timepoint_us),
                 next_back_gpu_end = double(cur_timing_info_.back_gpu_end_timepoint_us),
                 next_swap_start =
                     double(cur_timing_info_.swap_interval.start_timepoint_us),
            ...
            vertical_offset -= font_height;
            font_->DrawText(r, text_buffer, Ren::Vec2f{-1.0f, vertical_offset},
                            text_color, parent_);

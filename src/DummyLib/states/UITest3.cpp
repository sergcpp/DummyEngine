...
void UITest3::UpdateAnim(const uint64_t dt_us) {
    ...
    if (book_index_ != 0xffffffff) {
        ...
        if ((book->comp_mask & mask) == mask) {
            ...
            Ren::MeshCold &mesh_cold = ren_ctx_->storages().meshes[dr->mesh].second;
            Ren::Skeleton &skel = mesh_cold.skel;

            const int book_anim_index = int(book_state_);

            skel.UpdateAnim(book_anim_index, as->anim_time_s, ren_ctx_->storages().anims);
            skel.ApplyAnim(book_anim_index, ren_ctx_->storages().anims);

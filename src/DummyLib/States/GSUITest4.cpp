...
GSUITest4::GSUITest4(GameBase *game) : GSBaseState(game) {
    ...
    dialog_ui_.reset(
        new DialogUI{Gui::Vec2f{-1.0f, -1.0f}, Gui::Vec2f{2.0f, 2.0f}, ui_root_.get()});
    ...
    dialog_edit_ui_->set_cur_sequence_signal
        .Connect<GSUITest4, &GSUITest4::OnSetCurSequence>(this);
...
void GSUITest4::OnSetCurSequence(int id) {
    if (test_seq_) {

...
float calc_lnode_importance(const light_cwbvh_node_t n, const vec3 P, const vec3 inv_d, const vec3 neg_inv_d_o, const float t, const uint accept_mask, out float importance[8]) {
    const vec3 decode_ext = vec3(n.bbox_max[0] - n.bbox_min[0],
                                 n.bbox_max[1] - n.bbox_min[1],
                                 n.bbox_max[2] - n.bbox_min[2]) / 255.0;
    ...
    for (uint i = 0; i < 4; ++i) {
        float imp = n.flux[i];
        importance[i] = imp;
        if (((n.ch_bbox_min[0][0] >> (8u * i)) & 0xffu) != 0xff ||
            ((n.ch_bbox_max[0][0] >> (8u * i)) & 0xffu) != 0) {
            const vec3 bbox_min = vec3(n.bbox_min[0] + float((n.ch_bbox_min[0][0] >> (8u * i)) & 0xffu) * decode_ext[0],
                                       n.bbox_min[1] + float((n.ch_bbox_min[1][0] >> (8u * i)) & 0xffu) * decode_ext[1],
                                       n.bbox_min[2] + float((n.ch_bbox_min[2][0] >> (8u * i)) & 0xffu) * decode_ext[2]);
            const vec3 bbox_max = vec3(n.bbox_min[0] + float((n.ch_bbox_max[0][0] >> (8u * i)) & 0xffu) * decode_ext[0],
                                       n.bbox_min[1] + float((n.ch_bbox_max[1][0] >> (8u * i)) & 0xffu) * decode_ext[1],
                                       n.bbox_min[2] + float((n.ch_bbox_max[2][0] >> (8u * i)) & 0xffu) * decode_ext[2]);
            const float extent = 0.5 * length(bbox_max - bbox_min);
            ...
            importance[i] *= float((n.ch_bitmask[0] & (accept_mask << (8u * i))) != 0);
        }
        total_importance += imp;
    }
    for (uint i = 4; i < 8; ++i) {
        float imp = n.flux[i];
        importance[i] = imp;
        if (((n.ch_bbox_min[0][1] >> (8u * (i - 4u))) & 0xffu) != 0xff ||
            ((n.ch_bbox_max[0][1] >> (8u * (i - 4u))) & 0xffu) != 0) {
            const vec3 bbox_min = vec3(n.bbox_min[0] + float((n.ch_bbox_min[0][1] >> (8u * (i - 4u))) & 0xffu) * decode_ext[0],
                                       n.bbox_min[1] + float((n.ch_bbox_min[1][1] >> (8u * (i - 4u))) & 0xffu) * decode_ext[1],
                                       n.bbox_min[2] + float((n.ch_bbox_min[2][1] >> (8u * (i - 4u))) & 0xffu) * decode_ext[2]);
            const vec3 bbox_max = vec3(n.bbox_min[0] + float((n.ch_bbox_max[0][1] >> (8u * (i - 4u))) & 0xffu) * decode_ext[0],
                                       n.bbox_min[1] + float((n.ch_bbox_max[1][1] >> (8u * (i - 4u))) & 0xffu) * decode_ext[1],
                                       n.bbox_min[2] + float((n.ch_bbox_max[2][1] >> (8u * (i - 4u))) & 0xffu) * decode_ext[2]);
            const float extent = 0.5 * length(bbox_max - bbox_min);
            ...
            importance[i] *= float((n.ch_bitmask[1] & (accept_mask << (8u * (i - 4u)))) != 0);
        }
        total_importance += imp;
    }
    return total_importance;

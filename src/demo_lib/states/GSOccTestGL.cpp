...
namespace GSOccTestInternal {
	...
	inline GLuint attr(const ren::Program *p, int i) { return (GLuint)p->attribute(i).loc; }
	inline GLuint unif(const ren::Program *p, int i) { return (GLuint)p->uniform(i).loc; }
}
